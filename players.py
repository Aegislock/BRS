...
def get_stats(_name, stat_type='PER_GAME_STATS', playoffs=False, career=False, ask_matches = True):
    ...
    table = None
    if stat_type in ['per_game_stats', 'totals', 'advanced'] and not playoffs:
...
def get_player_salary(_name, option):
    ...
    if r.status_code == 200:
        ...
        df = df[df['Rk']!='Rk']
        df = df.drop(['Rk'], axis=1).reset_index(drop=True)
        if _name == 'all':
            df = df.dropna(subset=['Player'])
            return df
        else:
            df = df[df['Player'].str.contains(_name, case=False, na=False)]
        return df
    else:
        raise ConnectionError('Request to basketball reference failed')
